...
export default function UserProfilePage() {
    ...
    (async () => {
      ...
      if (uid && navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(async (pos) => {
          setMyProfile((prev: any) => prev ? { ...prev, latitude: pos.coords.latitude, longitude: pos.coords.longitude } : prev);
          await supabase.from("profiles").update({ latitude: pos.coords.latitude, longitude: pos.coords.longitude }).eq("user_id", uid);
        }, () => {});
      }
    })();
  }, [userId]);
  ...
  const handleLike = async () => {
    ...
    try {
      ...
      await supabase.from("swipes").insert({ from_id: myId, to_id: userId, action: "like" });
      const { data: match } = await supabase.from("matches").insert({ user_a: myId, user_b: userId }).select("id").single();
  ...
  const handleDislike = async () => {
    ...
    setActionLoading(true);
    try {
      await supabase.from("swipes").insert({ from_id: myProfile.user_id, to_id: userId, action: "dislike" });
      router.back();
    } catch (err) {
      console.error(err);
    } finally {
  ...
  const myPhoto = myProfile?.photo1_url ? photoSrc(myProfile.photo1_url) : null;
  const otherPhoto = photos[0] || null;
  const myName = myProfile?.nickname ?? myProfile?.first_name ?? "მე";
  ...
  return (
    <div className="min-h-[100dvh] bg-black text-white">
          ...
          <div className="absolute bottom-0 left-0 right-0 z-20 px-4 pb-4">
            ...
            <div className="flex justify-center gap-6 mt-4">
              <button onClick={handleDislike} disabled={actionLoading}
                className="w-16 h-16 rounded-full bg-black/60 backdrop-blur border-2 border-white/20 flex items-center justify-center text-2xl hover:bg-red-500/30 hover:border-red-500/50 active:scale-90 transition disabled:opacity-50 shadow-xl">
                ✕
              </button>
              <button onClick={handleLike} disabled={actionLoading}
                className="w-16 h-16 rounded-full bg-black/60 backdrop-blur border-2 border-white/20 flex items-center justify-center text-2xl hover:bg-pink-500/30 hover:border-pink-500/50 active:scale-90 transition disabled:opacity-50 shadow-xl">
                ❤️
              </button>
            </div>
          </div>
      ...
      {matchVisible && (
        <MatchModal
          onClose={() => { setMatchVisible(false); router.back(); }}
          onOpenChat={handleOpenChat}
          meName={myName}
          matchName={name}
          myPhoto={myPhoto}
          theirPhoto={otherPhoto}
        />
      )}
