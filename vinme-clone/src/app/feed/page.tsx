...
export default function FeedPage() {
  ...
  const loadMe = useCallback(async () => {
    ...
    // ✅ daily limits — reset თუ 24 საათი გავიდა
    const now = new Date();
    const isPlus = row?.is_plus && row?.plus_expires_at && new Date(row.plus_expires_at) > now;
 
    // Super Likes
    const slReset = row?.super_likes_reset_at ? new Date(row.super_likes_reset_at) : null;
    if (!slReset || now.getTime() - slReset.getTime() > 24 * 60 * 60 * 1000) {
      ...
      setSuperLikesLeft(Math.max(0, 1 - (row?.super_likes_used_today ?? 0)));
    }
 
    // First Impressions — Plus: 3/დღე, უფასო: 1/დღე
    const fiLimit = isPlus ? 3 : 1;
    const fiReset = row?.fi_reset_at ? new Date(row.fi_reset_at) : null;
    if (!fiReset || now.getTime() - fiReset.getTime() > 24 * 60 * 60 * 1000) {
      await supabase.from("profiles").update({
        fi_used_today: 0,
        fi_reset_at: now.toISOString(),
      }).eq("user_id", user.id);
      setFirstImpressionsLeft(fiLimit);
    } else {
      setFirstImpressionsLeft(Math.max(0, fiLimit - (row?.fi_used_today ?? 0)));
    }
  ...
  const loadTop = useCallback(async (myProfile: any) => {
    ...
    let query = supabase
      ...
      // ✅ incognito მომხმარებლები არ ჩანან
      .eq("incognito_mode", false)
      // ✅ discovery გამორთული არ ჩანან
      .eq("enable_discovery", true)
      // ✅ paused პროფილები არ ჩანან
      .eq("is_paused", false);
  ...
  const handleSuperLikeClick = async () => {
    if (superLikesLeft > 0) {
      ...
      // ✅ counter განახლება
      await supabase.from("profiles").update({
        super_likes_used_today: 1,
        super_likes_reset_at: new Date().toISOString(),
  ...
  const handleSendMessage = async () => {
    ...
    if (firstImpressionsLeft > 0) {
      ...
      advanceCard();
      await supabase.from("swipes").insert({ from_id: me.user_id, to_id: cur.user_id, action: "like" });
      await supabase.from("fi_messages").insert({ from_id: me.user_id, to_id: cur.user_id, message: msgText });
      // ✅ fi counter განახლება
      ...
      await supabase.from("profiles").update({
        fi_used_today: newUsed,
        fi_reset_at: new Date().toISOString(),
      }).eq("user_id", me.user_id);
  ...
  // ✅ hide_age და hide_distance — პროფილის მფლობელი მალავს, ჩვენ ვემორჩილებით
