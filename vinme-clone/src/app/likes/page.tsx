...
export default function LikesPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // Premium
      const { data: profile } = await supabase.from("profiles").select("is_premium, premium_until").eq("user_id", uid).maybeSingle();
      ...
      // ჩემი მეჩები + ვისაც მე დავულაიქე/დავუპასე, რომ მეორედ არ გამოჩნდეს
      const { data: myMatches } = await supabase.from("matches").select("user_a, user_b").or(`user_a.eq.${uid},user_b.eq.${uid}`);
      const { data: mySwipes } = await supabase.from("swipes").select("to_id").eq("from_id", uid);
      ...
      const { count } = await supabase.from("swipes").select("id", { count: "exact", head: true }).eq("to_id", uid).eq("action", "like");
      if (count !== null) {
        setLikeCount(count);
        localStorage.setItem("likes_count_cache", String(count));
      }
 
      const { data: swipes } = await supabase.from("swipes").select("from_id").eq("to_id", uid).eq("action", "like").limit(50);
 
      if (swipes && swipes.length > 0) {
        const fromIds = swipes.map((s: any) => s.from_id).filter((id: string) => !excludeIds.has(id)); 
        if (fromIds.length > 0) {
          const { data: profiles } = await supabase.from("profiles").select("user_id, nickname, age, photo1_url").in("user_id", fromIds);
    ...
    <main className="min-h-[100dvh] bg-black text-white pb-28">
      <div className="mx-auto w-full max-w-md px-4 pt-6">
        ...
        <div className="flex rounded-full bg-white/8 p-1 mb-6">
          <button className="flex-1 rounded-full py-2 text-sm font-semibold bg-white text-black">{L(`${likeCount} მოწონება`, `${likeCount} Like${likeCount !== 1 ? "s" : ""}`)}</button>
          <button className="flex-1 rounded-full py-2 text-sm font-semibold text-white/40">{L("საუკეთესო", "Top Picks")}<span className="ml-1 inline-block h-1.5 w-1.5 rounded-full bg-pink-500" /></button>
        </div>
 
        {loading ? (
          <div className="flex justify-center items-center py-20"><div className="w-8 h-8 border-2 border-pink-500 border-t-transparent rounded-full animate-spin" /></div>
        ) : isPremium ? (
          <div className="grid grid-cols-2 gap-3">
            {likers.length === 0 ? (
              <div className="col-span-2 text-center text-white/40 py-12">{L("ჯერ არავის მოუწონებია", "No likes yet")}</div>
            ) : (
              likers.map((liker) => (
                <div
                  key={liker.id}
           ...
           // დაბლარული ვერსია არა-პრემიუმისთვის
